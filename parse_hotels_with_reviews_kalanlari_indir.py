...
def main(data, region_name, download_dir ):
    ...
    """
    SADECE İNDİRİLMEMİŞ OLANLARI İNDİRMEYE ÇALIŞ 
    
    """

    df_indirilenler = pd.read_parquet(r"D:\calisma\projeler\tripadvisor\data\Florida\data\Florida\indirilmis_olanlar.parquet")
    indirilmis_olanlar= df_indirilenler[0].unique()
    ...
    for count in range(last_index, len(data)):#[hotelIDs[x] for x in [1,38, 39]]:
        ...
        if hotel_id in indirilmis_olanlar:
            print('\n\nBu Otel indirilmis. Pas geçiliyor\n\n')
